...
function validateSendMessageRequest(data) {
  ...
  if (
    !data.instance_name ||
    typeof data.instance_name !== 'string' ||
  ...
  if (
    ...
    typeof data.destination_number !== 'string' ||
    !data.destination_number.trim()
  ) {
    errors.push(
      'destination_number é obrigatório e deve ser uma string válida.'
    )
  }
...
router.post('/send_message', async (req, res) => {
  try {
    const errors = validateSendMessageRequest(req.body)
    if (errors.length > 0) {
      return res.status(400).json({ message: errors.join(' ') })
    }
    ...
    const payload = {
      number: req.body.destination_number,
